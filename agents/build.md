...
- **Verify for real.** Build and test the affected components using the project's own commands — find them in the build files, package scripts, Makefile targets or CI workflow definitions rather than guessing. Report the actual output. If it fails, fix it or report the failure verbatim — never claim success you did not observe.
...
- **`test`** — when the plan asks for test coverage beyond the obvious, or a test fails in a way you cannot diagnose.
